...
SELECT DISTINCT IbySysPmtProfilesVl.SYSTEM_PROFILE_NAME RES_NAME
  ...
  , (
        SELECT name
        FROM IBY_BEPINFO IbyBepinfo
        WHERE ibybepinfo.bepid = PS.bepid )                RES_PAYMENT_SYSTEM
  , DECODE(PS.TRANSMIT_INSTR_IMMED_FLAG,'Y','Yes','No')    RES_AUTOMATICALLY_TRANSMIT_PAY
  , PS.LAST_UPDATED_BY  RSC_LAST_UPDATED_BY
  , PS.LAST_UPDATE_DATE  RSC_LAST_UPDATE_DATE
  , PS.CREATED_BY  RSC_CREATED_BY
  , PS.CREATION_DATE  RSC_CREATION_DATE
  , null RSC_LEDGER_ID
...
WHERE PS.bepid is not null
and IbySysPmtProfilesVl.SYSTEM_PROFILE_CODE = PS.SYSTEM_PROFILE_CODE(+)

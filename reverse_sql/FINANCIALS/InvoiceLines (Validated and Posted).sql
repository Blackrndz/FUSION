...
,REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(InvoiceLinesE0.DESCRIPTION,chr(13),' '),chr(10),' '), '&', '#AMP#'), '<', '#OPENTAG#'), '>', '#CLOSETAG#') RES_DESCRIPTION
,(SELECT SEGMENT1
	FROM PO_HEADERS_ALL
...
,REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(InvoiceLinesE0.ITEM_DESCRIPTION,chr(13),' '),chr(10),' '), '&', '#AMP#'), '<', '#OPENTAG#'), '>', '#CLOSETAG#') RES_ITEM_DESCRIPTION
...
,FND_FLEX_EXT.GET_SEGS(APPLICATION_SHORT_NAME => 'GL'
	,KEY_FLEX_CODE => 'GL#'
	,STRUCTURE_NUMBER => (SELECT CHART_OF_ACCOUNTS_ID FROM GL_CODE_COMBINATIONS WHERE CODE_COMBINATION_ID = invDistsE0.DIST_CODE_COMBINATION_ID)
	,COMBINATION_ID => invDistsE0.DIST_CODE_COMBINATION_ID
	) RES_DISTRIBUTION_COMBINATION
,(SELECT SET_NAME
	FROM FND_SETID_SETS
...
,FND_FLEX_EXT.GET_SEGS(APPLICATION_SHORT_NAME => 'GL'
	,KEY_FLEX_CODE => 'GL#'
	,STRUCTURE_NUMBER => (SELECT CHART_OF_ACCOUNTS_ID FROM GL_CODE_COMBINATIONS WHERE CODE_COMBINATION_ID = InvoiceLinesE0.DEF_ACCTG_ACCRUAL_CCID)
	,COMBINATION_ID => InvoiceLinesE0.DEF_ACCTG_ACCRUAL_CCID
	) RES_MULTIPERIOD_ACCOUNTING_ACC
...
AND InvoicesE0.PAYMENT_STATUS_FLAG != 'Y'
...
,FND_FLEX_EXT.GET_SEGS(APPLICATION_SHORT_NAME => 'GL'
	,KEY_FLEX_CODE => 'GL#'
	,STRUCTURE_NUMBER => (SELECT CHART_OF_ACCOUNTS_ID FROM GL_CODE_COMBINATIONS WHERE CODE_COMBINATION_ID = InvoicesE0.ACCTS_PAY_CODE_COMBINATION_ID)
	,COMBINATION_ID => InvoicesE0.ACCTS_PAY_CODE_COMBINATION_ID
	) RES_DISTRIBUTION_COMBINATION
...
,AP_CHECKS_ALL ApChecksE0*/
WHERE InvoicesE0.INVOICE_ID = InvoiceLinesE0.INVOICE_ID
AND InvoicesE0.PAYMENT_STATUS_FLAG = 'P'

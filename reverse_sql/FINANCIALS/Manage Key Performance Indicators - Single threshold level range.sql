...
WITH TMPpjsPERFTHRESEO AS 
	(SELECT THRES_OBJ_ID,FROM_VALUE,TO_VALUE,INDICATOR_CODE,LAST_UPDATED_BY
	,LAST_UPDATE_DATE,CREATED_BY,CREATION_DATE,
	ROW_NUMBER() OVER (PARTITION BY THRES_OBJ_ID,CURRENCY_CODE ORDER BY FROM_VALUE,TO_VALUE ASC) AS ROW_NUM
	FROM PJS_PERF_THRESHOLDS )

SELECT pjsPERFRULEEO.RULE_NAME RES_KPI_NAME
,TMPpjsPERFTHRESEO.FROM_VALUE RES_THRESHOLD_START
,TMPpjsPERFTHRESEO.TO_VALUE RES_THRESHOLD_END
,'Within threshold range' RES_KPI_VALUE_COMPARED_TO_THRE
...
AND pjsPERFRULEEO.THRESHOLD_LEVEL = 'R'
...
SELECT pjsPERFRULEEO.RULE_NAME RES_KPI_NAME
,(SELECT FROM_VALUE FROM TMPpjsPERFTHRESEO WHERE THRES_OBJ_ID = pjsPERFRULEEO.RULE_ID AND ROW_NUM = 2) RES_THRESHOLD_START
,(SELECT TO_VALUE FROM TMPpjsPERFTHRESEO WHERE THRES_OBJ_ID = pjsPERFRULEEO.RULE_ID AND ROW_NUM = 2) RES_THRESHOLD_END
,'Outside threshold range' RES_KPI_VALUE_COMPARED_TO_THRE
,(SELECT MEANING FROM FND_LOOKUPS WHERE LOOKUP_CODE = TMPpjsPERFTHRESEO.INDICATOR_CODE 
	AND LOOKUP_TYPE = 'PJS_PERF_INDICATORS') RES_STATUS_INDICATOR

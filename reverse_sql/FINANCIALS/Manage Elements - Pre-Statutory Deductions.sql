 ...
 * $HeadURL: https://svn03.rapid4cloud.com/svn/a/dev/rapidesuite/controldata/FUSION_11.1.13/trunk/core/reverse_sql/FINANCIALS/Manage%20Elements%20-%20Pre-Statutory%20Deductions.sql $:
 * $Id: Manage Elements - Pre-Statutory Deductions.sql 78239 2022-04-07 15:37:05Z pisan.jariyasettachok $:
 * ****************************************************************************
 * Description:
 * ************************************************************************** */
...
,(CASE
	WHEN EXISTS (SELECT 1 FROM PAY_INPUT_VALUES_VL WHERE ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID AND NAME = 'Partial Flag' AND DEFAULT_VALUE = 'Y') THEN
		(CASE
			WHEN EXISTS (SELECT 1 FROM PAY_INPUT_VALUES_VL WHERE ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID AND NAME = 'Clear Arrears' AND DEFAULT_VALUE = 'Y') THEN
	...
	ELSE
		...
		END)
	END) RES_WHAT_SHOULD_HAPPEN_WHEN_TH	-- What should happen when there are insufficient funds to cover the deductions?	
...
,(CASE
	...
	END) RES_WHAT_IS_THE_CALCULATION_RU
,DECODE(elementsFE0.PRORATION_GROUP_ID,NULL,'No','Yes') RES_IS_THIS_ELEMENT_SUBJECT_TO	-- Is this element subject to proration?
,(SELECT EVENT_GROUP_NAME
    FROM PAY_EVENT_GROUPS_TL
    WHERE LANGUAGE = USERENV('LANG')
    AND EVENT_GROUP_ID = elementsE0.PRORATION_GROUP_ID
    ) RES_PRORATION_GROUP
,DECODE(elementsFE0.RECALC_EVENT_GROUP_ID,NULL,'No','Yes') RES_IS_THIS_ELEMENT_SUBJECT__0	-- Is this element subject to retroactive changes?
,(SELECT EVENT_GROUP_NAME
    FROM PAY_EVENT_GROUPS_TL
    WHERE LANGUAGE = USERENV('LANG')
...
,(CASE
	WHEN EXISTS (SELECT 1 FROM PAY_INPUT_VALUES_VL WHERE ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID AND NAME = 'Threshold Amount') THEN 
		'Yes'
	END) RES_PROCESSING_STOP_WHEN_THE_T  -- Processing Stop when the Total is reached?


-- On GB LEG
,(CASE
	WHEN prClassesE0.LEGISLATION_CODE = 'GB' THEN
		'Percentage deduction' 
	END) RES_WHAT_IS_THE_CALCULATION__1	-- What is the calculation rule for employee contributions?
,'' RES_WHAT_IS_THE_EMPLOYEE_PERCE	-- What is the employee percentage?
,(CASE
	WHEN prClassesE0.LEGISLATION_CODE = 'GB' THEN
		'Calendar Month'
	END) RES_WHAT_IS_THE_TIME_BASIS_FOR	-- What is the time basis for employee contributions?
,(CASE
	WHEN prClassesE0.LEGISLATION_CODE = 'GB' THEN
		'No'
	END) RES_DO_AGE_LIMITS_APPLY_TO_THI	-- Do age limits apply to this deduction?
,(CASE
	WHEN prClassesE0.LEGISLATION_CODE = 'GB' THEN
		'No'
	END) RES_DOES_A_MAXIMUM_CONTRIBUTIO	-- Does a maximum contribution limit apply to this deduction?
,(CASE
	WHEN prClassesE0.LEGISLATION_CODE = 'GB' THEN
		'No'
	END) RES_DO_PENSIONABLE_EARNINGS_LI	-- Do pensionable earnings limits apply to this deduction?
,(CASE
	WHEN prClassesE0.LEGISLATION_CODE = 'GB' THEN
		'No'
	END) RES_ARE_ADDITIONAL_CONTRIBUTIO	-- Are additional contributions allowed for this element?
,(CASE
	WHEN prClassesE0.LEGISLATION_CODE = 'GB' THEN
		'No'
	END) RES_ARE_EMPLOYER_CONTRIBUTIONS	-- Are employer contributions allowed for this deduction?
,(CASE
	WHEN prClassesE0.LEGISLATION_CODE = 'GB' THEN
		'Yes'
	END) RES_ARE_OVERRIDES_ALLOWED_FOR_	-- Are overrides allowed for this element?
,(CASE
	WHEN prClassesE0.LEGISLATION_CODE = 'GB' THEN
		'No'
	END) RES_IS_THIS_A_QUALIFYING_PENSI	-- Is this a Qualifying Pension Scheme?
...
AND prClassesE0.CLASSIFICATION_NAME = 'Pre-Statutory Deductions'

 ...
 * $HeadURL: https://svn03.rapid4cloud.com/svn/a/dev/rapidesuite/controldata/FUSION_11.1.13/trunk/core/reverse_sql/FINANCIALS/Manage%20Legal%20Entity%20(United%20States)%20-%20Manage%20Legal%20Entities.sql $:
 * $Id: Manage Legal Entity (United States) - Manage Legal Entities.sql 78399 2022-04-29 08:46:30Z tanawat.wongjan $:
 * ****************************************************************************
 * Description:
 * ************************************************************************** */
...
--, LocationDPEO.FORMATTED_ADDRESS         AS RES_LEGAL_ADDRESS
, XleRegistrations.PLACE_OF_REGISTRATION AS RES_PLACE_OF_REGISTRATION
, XleRegistrations.REGISTRATION_NUMBER   AS RES_EIN_OR_TIN
,(SELECT REGISTRATION_NUMBER
	FROM fusion.XLE_REGISTRATIONS
	WHERE SOURCE_TABLE = 'XLE_ETB_PROFILES'
	AND IDENTIFYING_FLAG = 'Y'
	AND LOCATION_ID = LocationDPEO.LOCATION_ID
	AND SOURCE_ID IN
		(SELECT ESTABLISHMENT_ID
		FROM fusion.XLE_ETB_PROFILES
		WHERE LEGAL_ENTITY_ID = xleentityprofiles.legal_entity_id
		AND GEOGRAPHY_ID = xleentityprofiles.GEOGRAPHY_ID
		AND ETB_INFORMATION_CONTEXT = xleentityprofiles.LE_INFORMATION_CONTEXT
...
AND TerritoryPEO.TERRITORY_CODE         = 'US'
ORDER BY RES_NAME
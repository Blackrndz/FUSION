 ...
 * $HeadURL: https://svn03.rapid4cloud.com/svn/a/dev/rapidesuite/controldata/FUSION_11.1.13/trunk/core/reverse_sql/FINANCIALS/Manage%20Elements%20-%20Ordinary%20Wages.sql $:
 * $Id: Manage Elements - Ordinary Wages.sql 78239 2022-04-07 15:37:05Z pisan.jariyasettachok $:
 * ****************************************************************************
 * Description:
 * ************************************************************************** */
...
,(CASE
	WHEN NOT EXISTS (SELECT 1 
			...
			AND cardComp.ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID
			AND elementsFE0.EFFECTIVE_END_DATE BETWEEN cardComp.EFFECTIVE_START_DATE AND cardComp.EFFECTIVE_END_DATE) THEN 
		DECODE(elementsE0.STANDARD_LINK_FLAG ,'Y','Yes','N','No') 
	END) RES_SHOULD_EVERY_PERSON_ELIGIB 	-- Should every person eligible for the element automatically receive it?
...
,(CASE
    ...
    END) RES_DOES_THIS_ELEMENT_RECUR_EA 	-- Does this element recur each payroll period, or does it require explicit entry?
...
,(CASE
	WHEN NOT EXISTS (SELECT 1 
			...
			AND cardComp.ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID
			AND elementsFE0.EFFECTIVE_END_DATE BETWEEN cardComp.EFFECTIVE_START_DATE AND cardComp.EFFECTIVE_END_DATE) THEN 
		DECODE(elementsE0.ONCE_EACH_PERIOD_FLAG,'Y','Yes','N','No')
	END) RES_PROCESS_THE_ELEMENT_ONLY_O -- Process the element only once in each payroll period?
...
,(CASE
	WHEN NOT EXISTS (SELECT 1 
			...
			AND cardComp.ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID
			AND elementsFE0.EFFECTIVE_END_DATE BETWEEN cardComp.EFFECTIVE_START_DATE AND cardComp.EFFECTIVE_END_DATE) THEN 
		DECODE(elementsE0.MULTIPLE_ENTRIES_ALLOWED_FLAG ,'Y','Yes','N','No')
	END) RES_CAN_A_PERSON_HAVE_MORE_THA 	-- Can a person have more than one entry of this element in a payroll period?
,(CASE
	WHEN NOT EXISTS (SELECT 1 
			FROM PAY_DIR_CARD_COMP_DEFS_F cardComp
			,PAY_DIR_CARD_DEFINITIONS cardDef
...
,(CASE
	WHEN NOT EXISTS (SELECT 1 
			...
			AND cardComp.ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID
			AND elementsFE0.EFFECTIVE_END_DATE BETWEEN cardComp.EFFECTIVE_START_DATE AND cardComp.EFFECTIVE_END_DATE) THEN 
		DECODE(elementsFE0.PRORATION_GROUP_ID,NULL,'No','Yes')
...
,(CASE
	WHEN NOT EXISTS (SELECT 1 
			...
			AND cardComp.ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID
			AND elementsFE0.EFFECTIVE_END_DATE BETWEEN cardComp.EFFECTIVE_START_DATE AND cardComp.EFFECTIVE_END_DATE) THEN 
		DECODE(elementsFE0.RECALC_EVENT_GROUP_ID,NULL,'No','Yes') 
...
AND prClassesE0.CLASSIFICATION_NAME = 'Ordinary Wages'

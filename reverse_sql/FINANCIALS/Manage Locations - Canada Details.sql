 ...
 * $HeadURL: https://svn03.rapid4cloud.com/svn/a/dev/rapidesuite/controldata/FUSION_11.1.13/trunk/core/reverse_sql/FINANCIALS/Manage%20Locations%20-%20Canada%20Details.sql $:
 * $Id: Manage Locations - Canada Details.sql 80146 2023-02-10 10:05:41Z tanawat.wongjan $:
 * ****************************************************************************
 * Description:
 * ************************************************************************** */
...
,(SELECT MEANING
	FROM HR_LOOKUPS
	WHERE LOOKUP_TYPE = 'HRX_CA_CMA_CODE'
	AND ENABLED_FLAG  = 'Y'
	AND LOOKUP_CODE   = Census.lle_information1
	) RES_CENSUS_METROPOLITAN_AREA
, NULL RES_WORKERS_COMPENSATION_CLASS
	--Value have not been added yet.
...
,(SELECT AAA.location_id
	...
	,BBB.EFFECTIVE_START_DATE EFFECTIVE_START_DATE_1
	,BBB.EFFECTIVE_END_DATE EFFECTIVE_END_DATE_2
	FROM fusion.PER_LOCATION_LEG_F AAA
	,fusion.PER_LOCATION_DETAILS_F BBB
	WHERE AAA.location_id            = BBB.location_id
	AND AAA.lle_information_category = 'HRX_CA_LOC_EER_RULES'
...
WHERE((HEAD.location_id = Census.location_id(+))
AND(HEAD.EFFECTIVE_START_DATE BETWEEN Census.EFFECTIVE_START_DATE_1(+) AND Census.EFFECTIVE_END_DATE_2(+)
))

 ...
 * $HeadURL: https://svn03.rapid4cloud.com/svn/a/dev/rapidesuite/controldata/FUSION_11.1.13/trunk/core/reverse_sql/FINANCIALS/Manage%20Elements%20-%20Voluntary%20Deductions.sql $:
 * $Id: Manage Elements - Voluntary Deductions.sql 78239 2022-04-07 15:37:05Z pisan.jariyasettachok $:
 * ****************************************************************************
 * Description:
 * ************************************************************************** */
...
,(SELECT EVENT_GROUP_NAME
    ...
    ) RES_PRORATION_GROUP 			-- Proration Group
...
,(SELECT EVENT_GROUP_NAME
    ...
    ) RES_RETRO_GROUP 				-- Retro Group


,'' RES_DO_YOU_WANT_TO_STOP_PROCES -- Do you want to stop processing when the total owed is reached? If you enable total owed, a reference is required for each deduction entry.
,(CASE
	WHEN legislativeDataGrpsE0.LEGISLATION_CODE = 'CA' THEN
		(SELECT inp.DEFAULT_VALUE
			FROM PAY_INPUT_VALUES_VL inp
			WHERE inp.NAME = 'Registration Number' 
			AND inp.ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID)
	END) RES_WHAT_IS_THE_REGISTRATION_N -- What is the Registration Number for this element?
,(CASE
	WHEN legislativeDataGrpsE0.LEGISLATION_CODE = 'CA' THEN
		(SELECT lk.MEANING
			FROM PAY_INPUT_VALUES_VL inp
...
,'' RES_IS_THIS_ELEMENT_SUBJECT__1	-- Is this element subject to iterative processing?
,'' RES_WHAT_IS_THE_DEFAULT_PERIOD	-- What is the default periodicity of this element?
,'' RES_HOW_DO_YOU_WANT_THE_WORK_U	-- How do you want the work units to be reported?
,'' RES_USE_THIS_ELEMENT_TO_CALCUL	-- Use this element to calculate a gross amount from a specified net amount?
,'' RES_PERIODICITY_CONVERSION_RUL	-- Periodicity Conversion Rule
,'' RES_PROCESS_AND_PAY_ELEMENT_SE	-- Process and pay element separately or with other earnings elements?
...
AND prClassesE0.BASE_CLASSIFICATION_NAME = 'Voluntary Deductions'

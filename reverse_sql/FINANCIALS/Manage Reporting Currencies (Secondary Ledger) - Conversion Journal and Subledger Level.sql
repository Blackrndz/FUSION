 ...
 * $HeadURL: https://svn03.rapid4cloud.com/svn/a/dev/rapidesuite/controldata/FUSION_11.1.13/trunk/core/reverse_sql/FINANCIALS/Manage%20Reporting%20Currencies%20(Secondary%20Ledger)%20-%20Conversion%20Journal%20and%20Subledger%20Level.sql $:
 * $Id: Manage Reporting Currencies (Secondary Ledger) - Conversion Journal and Subledger Level.sql 78229 2022-04-07 07:37:05Z pisan.jariyasettachok $:
 * ****************************************************************************
 * Description:
 * ************************************************************************** */
...
,(GlLedgerRelationships.TARGET_CURRENCY_CODE || ' - ' || (select CurrencyPEO.NAME FROM fnd_currencies_vl CurrencyPEO WHERE CurrencyPEO.CURRENCY_CODE = GlLedgerRelationships.TARGET_CURRENCY_CODE)) AS RES_CURRENCY
...
,DECODE(GlLedgerRelationships.RELATIONSHIP_TYPE_CODE,'BALANCE',null,nvl((SELECT MEANING
	...
	),'No sequencing')) as RES_SEQUENCING_BY	
,DECODE(GlLedgerRelationships.RELATIONSHIP_TYPE_CODE,'BALANCE',null,DECODE(
	(SELECT AP_DOC_SEQUENCING_OPTION_FLAG FROM GL_LEDGERS WHERE LEDGER_ID = GlLedgerRelationships.SLA_LEDGER_ID
	),'Y','No',DECODE(
	(SELECT AR_DOC_SEQUENCING_OPTION_FLAG FROM GL_LEDGERS WHERE LEDGER_ID = GlLedgerRelationships.SLA_LEDGER_ID
...
,DECODE(GlLedgerRelationships.RELATIONSHIP_TYPE_CODE,'BALANCE',null,nvl((SELECT MEANING
	...
	),'No')) AS RES_ENFORCE_CHRONOLOGICAL_ORDE
...
AND GlLedgerRelationships.SLA_LEDGER_ID in (select LEDGER_ID from GL_LEDGERS where LEDGER_CATEGORY_CODE = 'SECONDARY')
AND GlLedgerRelationships.RELATIONSHIP_TYPE_CODE = 'JOURNAL'

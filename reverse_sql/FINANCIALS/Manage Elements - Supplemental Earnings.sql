 ...
 * $HeadURL: https://svn03.rapid4cloud.com/svn/a/dev/rapidesuite/controldata/FUSION_11.1.13/trunk/core/reverse_sql/FINANCIALS/Manage%20Elements%20-%20Supplemental%20Earnings.sql $:
 * $Id: Manage Elements - Supplemental Earnings.sql 78239 2022-04-07 15:37:05Z pisan.jariyasettachok $:
 * ****************************************************************************
 * Description:
 * ************************************************************************** */
...
,(CASE
	WHEN EXISTS (SELECT 1 FROM PAY_INPUT_VALUES_VL WHERE ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID AND NAME = 'Earned Start Date') THEN 
		'Yes'
	ELSE
		'No'
	END) RES_PRORATE_THIS_EARNING_ACROS	-- Prorate this earning across all periods during which it was earned, and consider it for FLSA calculations, such as for commissions, bonuses, incentives, and other nondiscretionary earnings?
...
,(SELECT ff.FORMULA_NAME
	FROM PAY_INPUT_VALUES_VL inp
	,FF_FORMULAS_VL ff
	WHERE inp.NAME = 'Proration Conversion Rule' 
	AND inp.DEFAULT_VALUE = ff.BASE_FORMULA_NAME
	AND inp.ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID 
	) RES_PRORATION_RATE_CONVERSION_	-- Proration Rate Conversion Rule
,(SELECT DEFAULT_VALUE 
	FROM PAY_INPUT_VALUES_VL 
	WHERE NAME = 'Proration Unit' 
	AND ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID 
	) RES_PRORATION_UNITS				-- Proration Units
...
,'No' RES_SHOULD_THIS_ELEMENT_BE_INC	-- Should this element be included in the earnings calculation of the FLSA overtime base rate?
,(CASE
	WHEN EXISTS (SELECT 1 FROM PAY_INPUT_VALUES_VL WHERE ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID AND NAME = 'Unit Type' AND DEFAULT_VALUE = 'H') THEN 
		'Yes'
	END) RES_SHOULD_THIS_ELEMENT_BE_I_2	-- Should this element be included in the hours calculation of the FLSA overtime base rate?
,'No' RES_DOES_THIS_ELEMENT_HAVE_A_D	-- Does this element have a default rate definition?
,'' RES_WORK_UNITS_CONVERSION_RU_3	-- Work Units Conversion Rule?
,'Hours' RES_WHAT_ARE_THE_CALCULATION_U	-- What are the calculation units for reporting?
...
,(SELECT lk.MEANING
	...
	AND inp.ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID 
	) RES_WHAT_TAX_WITHHOLDING_METHO -- What tax withholding method applies to this Supplemental Earning element?
,(SELECT lk.MEANING
	FROM PAY_INPUT_VALUES_VL inp
	,FND_LOOKUP_VALUES lk
	WHERE lk.LANGUAGE = USERENV('LANG')
	AND lk.LOOKUP_TYPE = inp.LOOKUP_TYPE
	AND lk.LOOKUP_CODE = inp.DEFAULT_VALUE
	AND inp.NAME = 'Commission' 
	AND inp.ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID 
	) RES_IS_THIS_A_COMMISSION_ELEME -- Is this a commission element?
,(SELECT lk.MEANING
	FROM PAY_INPUT_VALUES_VL inp
	,FND_LOOKUP_VALUES lk
	WHERE lk.LANGUAGE = USERENV('LANG')
...
,(SELECT lk.MEANING
	...
	AND lk.LOOKUP_TYPE = inp.LOOKUP_TYPE
	AND lk.LOOKUP_CODE = inp.DEFAULT_VALUE
	AND inp.NAME = 'Vacation Payout' 
	AND inp.ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID 
	) RES_IS_THIS_ELEMENT_USED_TO_PA -- Is this element used to pay out vacation liability?
,'' RES_RATE_NAME
...
AND prClassesE0.CLASSIFICATION_NAME = 'Supplemental Earnings'

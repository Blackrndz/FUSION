 ...
 * $HeadURL: https://svn03.rapid4cloud.com/svn/a/dev/rapidesuite/controldata/FUSION_11.1.13/trunk/core/reverse_sql/FINANCIALS/Manage%20Elements%20-%20Imputed%20Earnings.sql $:
 * $Id: Manage Elements - Imputed Earnings.sql 78239 2022-04-07 15:37:05Z pisan.jariyasettachok $:
 * ****************************************************************************
 * Description:
 * ************************************************************************** */
...
,(CASE
	WHEN EXISTS (SELECT 1 FROM PAY_INPUT_VALUES_VL inp
	WHERE inp.NAME = 'Taxation Pay Periods' 
	AND inp.ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID)
	THEN 'Yes'
	ELSE 'No'
	END) RES_TAX_THIS_EARNING_ACROSS_MU
,(CASE
...
,'No' RES_SHOULD_THIS_ELEMENT_BE_I_1	-- Should this element be included in the hours calculation of the FLSA overtime base rate?
...
,(CASE
	WHEN EXISTS (SELECT 1 
			...
			AND elementsFE0.EFFECTIVE_END_DATE BETWEEN cardComp.EFFECTIVE_START_DATE AND cardComp.EFFECTIVE_END_DATE) THEN 
			'No'
			ELSE NULL END)RES_DOES_THIS_ELEMENT_HAVE_A_D	-- Does this element have a default rate definition?
,NULL RES_RATE_NAME
...
AND prClassesE0.CLASSIFICATION_NAME = 'Imputed Earnings'
...
/*AND (SELECT COUNT(1) CNT 
	...
	WHERE BASE_ELEMENT_NAME LIKE elementsFE0.BASE_ELEMENT_NAME||'%') > 1*/

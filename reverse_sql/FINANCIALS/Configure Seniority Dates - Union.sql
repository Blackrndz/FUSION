...
,TRIM(REGEXP_SUBSTR(seniorityDatesE0.SELECT_UNION_NAME, '[^,]+', 1, levels.column_value)) RES_UNION 
,'Yes' RES_UNION_VALUE
...
,table(cast(multiset(select level from dual connect by  level <= length (regexp_replace(seniorityDatesE0.SELECT_UNION_NAME, '[^,]+'))  + 1) as sys.OdciNumberList)) levels
WHERE seniorityDatesE0.SELECT_UNION_NAME IS NOT NULL
ORDER BY seniorityDatesE0.SENIORITY_DATE_CODE,RES_UNION
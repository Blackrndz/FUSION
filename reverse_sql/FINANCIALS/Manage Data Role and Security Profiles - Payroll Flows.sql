...
,PayFlowSecurityProfileEO.Name RES_NAME_BASIC_DETAILS
,PayFlowSecurityProfileEO.FLOW_NAME RES_PAYROLL_FLOW
...
,PayFlowSecurityProfileEO.BUSINESS_GROUP_ID RSC_BUSINESS_GROUP_ID
...
,(SELECT PayFlow.Name
	,FlowPEO.FLOW_NAME
	,PayFlowSecProfilePayEO.CREATED_BY
	,PayFlowSecProfilePayEO.CREATION_DATE
	,PayFlowSecProfilePayEO.LAST_UPDATED_BY
	,PayFlowSecProfilePayEO.LAST_UPDATE_DATE
	,PayFlowSecProfilePayEO.BUSINESS_GROUP_ID
	,DataE0.GENERATED_DATA_ROLE_ID
	FROM PAY_FLW_SECURITY_PROFILES PayFlow
	,PAY_FLW_SECURITY_PROFILE_PAYS PayFlowSecProfilePayEO
	,PAY_FLOWS_VL FlowPEO
	,PER_GEN_DATA_ROLE_PROFILES DataE0
	WHERE DataE0.SECURITY_PROFILE_ID = PayFlow.FLW_SECURITY_PROFILE_ID
	AND PayFlow.FLW_SECURITY_PROFILE_ID = PayFlowSecProfilePayEO.FLW_SECURITY_PROFILE_ID
	AND PayFlowSecProfilePayEO.FLOW_ID = FlowPEO.FLOW_ID
	AND DataE0.HR_SECURING_OBJECT    = 'FLOWPATTERN'
	) PayFlowSecurityProfileEO
WHERE PerRole.ROLE_ID                     = PerGenDataRole.DATA_ROLE_ID
AND PerGenDataRole.GENERATED_DATA_ROLE_ID = PayFlowSecurityProfileEO.GENERATED_DATA_ROLE_ID

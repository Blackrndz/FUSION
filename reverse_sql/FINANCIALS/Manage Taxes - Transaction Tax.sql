...
,DECODE(TaxTLEO.ZONE_GEOGRAPHY_TYPE,'ECONOMIC REGIONS','Tax zone','Country') RES_REGIME_LEVEL
,(SELECT MEANING
	FROM FND_LOOKUPS
	WHERE LOOKUP_TYPE = 'ZX_TAX_TYPE_CATEGORY'
	AND LOOKUP_CODE   = TaxTLEO.TAX_TYPE_CODE
	) RES_TAX_TYPE
,TO_CHAR(TaxTLEO.EFFECTIVE_FROM,'DD-Mon-YYYY') RES_START_DATE
,TO_CHAR(TaxTLEO.EFFECTIVE_TO,'DD-Mon-YYYY') RES_END_DATE
,DECODE(TaxTLEO.LIVE_FOR_PROCESSING_FLAG,'Y','Yes','S','Yes','No') RES_ENABLE_TAX_FOR_SIMULATION
...
,RepAuth.PARTY_NAME RES_REPORTING_TAX_AUTHORITY
,CollAuth.PARTY_NAME RES_COLLECTING_TAX_AUTHORITY
,(SELECT MEANING
	FROM FND_LOOKUPS
	WHERE LOOKUP_TYPE = 'ZX_APPLIED_AMT_HANDLING'
	AND LOOKUP_CODE   = TaxTLEO.APPLIED_AMT_HANDLING_FLAG
	) RES_APPLIED_AMOUNT_HANDLING
,DECODE(TaxTLEO.OFFSET_TAX_FLAG,'Y','Yes','No') RES_SET_AS_OFFSET_TAX
,DECODE(TaxTLEO.REPORTING_ONLY_FLAG,'Y','Yes','No') RES_SET_TAX_FOR_REPORTING_PURP
...
AND RepPTP.PARTY_ID                              = RepAuth.PARTY_ID(+)
AND TaxTLEO.COLL_TAX_AUTHORITY_ID                = CollPTP.PARTY_TAX_PROFILE_ID(+)
AND CollPTP.PARTY_ID                             = CollAuth.PARTY_ID(+)

...
,breaksE0.NAME RES_BREAK_NAME
,(SELECT MEANING
	FROM FND_LOOKUP_VALUES
	WHERE LANGUAGE = USERENV('LANG')
	AND LOOKUP_TYPE = 'ORA_ANC_BREAK_TYPES'
	AND LOOKUP_CODE = breaksE0.BREAK_TYPE_CODE 
	) RES_BREAK_TYPE
,breaksE0.BREAK_DURATION RES_DURATION
,(SELECT MEANING
	FROM FND_LOOKUP_VALUES
	WHERE LANGUAGE = USERENV('LANG')
	AND LOOKUP_TYPE = 'ORA_ANC_BREAK_IS_PAID'
	AND LOOKUP_CODE = breaksE0.BREAK_PAID 
	) RES_NATURE_OF_THE_BREAK
,breaksE0.LAST_UPDATED_BY  RSC_LAST_UPDATED_BY
,breaksE0.LAST_UPDATE_DATE  RSC_LAST_UPDATE_DATE
,breaksE0.CREATED_BY  RSC_CREATED_BY
,breaksE0.CREATION_DATE  RSC_CREATION_DATE
,null RSC_LEDGER_ID
...
,ANC_AVAIL_TYPE_BREAKS_VL breaksE0
WHERE avalTypesE0.AVAILABILITY_TYPE_ID = breaksE0.AVAILABILITY_TYPE_ID

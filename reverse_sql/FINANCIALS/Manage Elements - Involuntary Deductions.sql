 ...
 * $HeadURL: https://svn03.rapid4cloud.com/svn/a/dev/rapidesuite/controldata/FUSION_11.1.13/trunk/core/reverse_sql/FINANCIALS/Manage%20Elements%20-%20Involuntary%20Deductions.sql $:
 * $Id: Manage Elements - Involuntary Deductions.sql 78239 2022-04-07 15:37:05Z pisan.jariyasettachok $:
 * ****************************************************************************
 * Description:
 * ************************************************************************** */
...
,NULL RES_SHOULD_EVERY_PERSON_ELIGIB 	-- Should every person eligible for the element automatically receive it?
...
,NULL RES_AT_WHICH_EMPLOYMENT_LEVEL_ 	-- At which employment level should this element be attached?
,NULL RES_DOES_THIS_ELEMENT_RECUR_EA 	-- Does this element recur each payroll period, or does it require explicit entry?

,NULL RES_CAN_A_PERSON_HAVE_MORE_THA 	-- Can a person have more than one entry of this element in a payroll period?
,NULL RES_PROCESS_THE_ELEMENT_ONLY_O 	-- Process the element only once in each payroll period?
,(CASE
	WHEN EXISTS (SELECT 1 FROM PAY_INPUT_VALUES_VL WHERE ELEMENT_TYPE_ID = elementsFE0.ELEMENT_TYPE_ID AND NAME = 'TowardsOwed' AND DEFAULT_VALUE = 'Y') THEN 
		'Yes'
	ELSE
		'No'
	END) RES_PROCESSING_STOP_WHEN_THE_T
...
AND prClassesE0.CLASSIFICATION_NAME = 'Involuntary Deductions'
...
/*AND (elementsE0.ELEMENT_NAME  NOT LIKE '%Adjustment'
	AND elementsE0.ELEMENT_NAME  NOT LIKE '%Deduction Results'
	AND elementsE0.ELEMENT_NAME  NOT LIKE '%Processor'
	AND elementsE0.ELEMENT_NAME  NOT LIKE '%Calculator'
	AND elementsE0.ELEMENT_NAME  NOT LIKE '%Common Calculation Rules'
	AND elementsE0.ELEMENT_NAME  NOT LIKE '%Fee Processor'
	AND elementsE0.ELEMENT_NAME  NOT LIKE '%Maintenance'
	AND elementsE0.ELEMENT_NAME  NOT LIKE '%Organization Fee Results'
	AND elementsE0.ELEMENT_NAME  NOT LIKE '%Person Fee Results'
	AND elementsE0.ELEMENT_NAME  NOT LIKE '%Processing Fee Results'
	AND elementsE0.ELEMENT_NAME  NOT LIKE '%Results'
	AND elementsE0.ELEMENT_NAME  NOT LIKE '%Student Loan Arrears'
	AND elementsE0.ELEMENT_NAME  NOT LIKE '%Balance Feed')*/

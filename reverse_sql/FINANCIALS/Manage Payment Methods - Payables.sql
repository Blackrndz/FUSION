...
SELECT IbyPaymentMethodsTl.PAYMENT_METHOD_NAME RES_NAME ,
  ...
  DECODE(
  (SELECT distinct IbyApplicablePmtMthds.APPLICABLE_TYPE_CODE FROM IBY_APPLICABLE_PMT_MTHDS IbyApplicablePmtMthds WHERE IbyApplicablePmtMthds.APPLICATION_ID = 200
  ...
  ),'PAYER_ORG','Yes','No') RES_ENABLE_FOR_USE_IN_PAYABLES ,
  (SELECT meaning
...
FROM IBY_PAYMENT_METHODS_TL IbyPaymentMethodsTl ,
  ...
  AND ( IbyApplicablePmtMthds.APPLICATION_ID       = 200 ) ) )
  ) SUB_detail ,
  (SELECT IbyApplicablePmtMthds.PAYMENT_METHOD_CODE ,
    IbyApplicablePmtMthds.APPLICABLE_VALUE_TO ,
    LAST_UPDATED_BY ,

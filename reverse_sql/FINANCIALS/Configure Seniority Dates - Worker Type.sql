...
,(SELECT MEANING
    ...
    AND LOOKUP_CODE = (TRIM(REGEXP_SUBSTR(seniorityDatesE0.SELECT_WORKER_TYPE, '[^,]+', 1, levels.column_value)))
	) RES_WORKER_TYPE
,'Yes' RES_WORKER_TYPE_VALUE
...
,table(cast(multiset(select level from dual connect by  level <= length (regexp_replace(seniorityDatesE0.SELECT_WORKER_TYPE, '[^,]+'))  + 1) as sys.OdciNumberList)) levels
WHERE seniorityDatesE0.SELECT_WORKER_TYPE IS NOT NULL
ORDER BY seniorityDatesE0.SENIORITY_DATE_CODE,RES_WORKER_TYPE
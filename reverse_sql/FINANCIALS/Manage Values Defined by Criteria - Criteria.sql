...
,NULL LOW_VALUE_TEXT
,NULL HIGH_VALUE
,NULL HIGH_VALUE_TEXT
,NULL VALUE1
,NULL VALUE2
,NULL VALUE3
,NULL  AS ITEM_START_DATE
,NULL  AS ITEM_END_DATE
,NULL  AS ITEM_CRE_BY
,NULL  AS ITEM_CRE_DATE
,NULL  AS ITEM_LAST_DATE
,NULL  AS ITEM_LAST_BY
								FROM PAY_VALUE_DEFINITIONS_F payVALUESDEFINIEO
								,PAY_VALUE_DEFINITIONS_TL payVALUESDEFINITLEO
								WHERE payVALUESDEFINITLEO.LANGUAGE = USERENV('LANG')
								AND payVALUESDEFINIEO.VALUE_DEFN_ID = payVALUESDEFINITLEO.VALUE_DEFN_ID
								AND payVALUESDEFINIEO.DEFAULT_FLAG = 'Y'
								)
,TreeE0 AS 
...
,RES_CREATE_VALUE_VALUE_IDENTIF
,RES_VALUE_DEFINITION_GROUP
,RES_RETRIEVAL_DATE
,RES_CRITERIA_START_DATE
,RES_CRITERIA_END_DATE
,RES_CONDITION_DEFAULT_CRITERIA
,RES_CONDITION_SEQUENCE
,RES_CONDITION_DATABASE_ITEM_NA
,RES_CONDITION_DISPLAY_NAME
,RES_CONDITION_OPERAND
,RES_CONDITION_VALUE_SET
...
,(CASE
	...
	END) RES_PARENT_5
,(CASE WHEN tmpPAY.DISPLAY_TAG IS NULL AND tmpPAY.LOW_VALUE_TEXT IS NOT NULL AND tmpPAY.VALUE_SET_CODE1 IS NULL
	THEN tmpPAY.DB_DESC||' '||tmpPAY.EXPRESSION||' '||tmpPAY.LOW_VALUE_TEXT
	WHEN tmpPAY.DISPLAY_TAG IS NOT NULL AND tmpPAY.LOW_VALUE_TEXT IS NOT NULL AND tmpPAY.VALUE_SET_CODE1 IS NULL
	THEN tmpPAY.DISPLAY_TAG||' '||tmpPAY.EXPRESSION||' '||tmpPAY.LOW_VALUE_TEXT
	WHEN tmpPAY.DISPLAY_TAG IS NULL AND tmpPAY.LOW_VALUE_TEXT IS NOT NULL AND tmpPAY.VALUE_SET_CODE1 IS NOT NULL 
	THEN tmpPAY.DB_DESC||' '||tmpPAY.EXPRESSION||' '||'###R4C###'||tmpPAY.VALUE_SET_CODE1||'###R4CVS###'||tmpPAY.LOW_VALUE_TEXT||'###R4C###'
	WHEN tmpPAY.DISPLAY_TAG IS NOT NULL AND tmpPAY.LOW_VALUE_TEXT IS NOT NULL AND tmpPAY.VALUE_SET_CODE1 IS NOT NULL
	THEN tmpPAY.DISPLAY_TAG||' '||tmpPAY.EXPRESSION||' '||'###R4C###'||tmpPAY.VALUE_SET_CODE1||'###R4CVS###'||tmpPAY.LOW_VALUE_TEXT||'###R4C###'
	WHEN DEFAULT_FLAG = 'Y'
	THEN 'Default Criteria' END) RES_CRITERIA_OR_VALUE_DISPLAYE
,'Criteria' RES_CALCULATION_VALUE_DEFINITI
,tmpPAY.NAME RES_CALCULATION_VALUE_DEFINI_0
,tmpPAY.VALUE_IDENTIFIER RES_CREATE_VALUE_VALUE_IDENTIF
...
,TO_CHAR(tmpPAY.EFFECTIVE_END_DATE,'DD-Mon-YYYY') RES_CRITERIA_END_DATE
,DECODE(tmpPAY.DEFAULT_FLAG,'Y','Yes','No') RES_CONDITION_DEFAULT_CRITERIA
,tmpPAY.SEQUENCE RES_CONDITION_SEQUENCE
,(SELECT USER_NAME FROM FF_DATABASE_ITEMS_VL WHERE DATABASE_ITEM_ID = tmpPAY.DATABASE_ITEM_ID) RES_CONDITION_DATABASE_ITEM_NA
,tmpPAY.DISPLAY_TAG RES_CONDITION_DISPLAY_NAME
,(SELECT MEANING FROM FND_LOOKUP_VALUES WHERE LOOKUP_CODE = tmpPAY.EXPRESSION
		AND LOOKUP_TYPE = 'PAY_DED_VALUE_EXPRESSION' AND LANGUAGE = USERENV('LANG')
		AND SYSDATE BETWEEN START_DATE_ACTIVE AND NVL(END_DATE_ACTIVE ,TO_DATE('31-12-4712','DD-MM-YYYY'))) RES_CONDITION_OPERAND
,tmpPAY.VALUE_SET_CODE1 RES_CONDITION_VALUE_SET

...
,(SELECT TERRITORY_SHORT_NAME 
	...
	WHERE TERRITORY_CODE = (TRIM(REGEXP_SUBSTR(seniorityDatesE0.SELECT_LEGISLATION_CODE, '[^,]+', 1, levels.column_value)))
	) RES_COUNTRY 
,'Yes' RES_COUNTRY_VALUE
...
,table(cast(multiset(select level from dual connect by  level <= length (regexp_replace(seniorityDatesE0.SELECT_LEGISLATION_CODE, '[^,]+'))  + 1) as sys.OdciNumberList)) levels
WHERE seniorityDatesE0.SELECT_LEGISLATION_CODE IS NOT NULL
ORDER BY seniorityDatesE0.SENIORITY_DATE_CODE,RES_COUNTRY
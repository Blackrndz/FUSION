...
,TO_CHAR(balancefeedE0.EFFECTIVE_START_DATE,'DD-Mon-YYYY') RES_EFFECTIVE_START_DATE
,(SELECT NAME FROM PAY_INPUT_VALUES_VL WHERE INPUT_VALUE_ID = balancefeedE0.INPUT_VALUE_ID
	AND SYSDATE BETWEEN EFFECTIVE_START_DATE AND NVL(EFFECTIVE_END_DATE,TO_DATE('4712-12-31','YYYY-MM-DD'))) RES_NAME
,balancetypeEO.BALANCE_NAME RES_BALANCE_NAME
,(SELECT MEANING
    FROM FND_LOOKUP_VALUES
    WHERE LANGUAGE = USERENV('LANG')
    AND LOOKUP_TYPE = 'PAY_ADD_SUBTRACT'
    AND LOOKUP_CODE = balancefeedE0.SCALE
    )  RES_ADD_OR_SUBTRACT
,(SELECT MEANING
    FROM FND_LOOKUP_VALUES
    WHERE LANGUAGE = USERENV('LANG')
    AND LOOKUP_TYPE = 'PAY_UNITS'
    AND LOOKUP_CODE = balancetypeEO.BALANCE_UOM 
    )  RES_UNIT_OF_MEASURE
,DECODE(TO_CHAR(balancefeedE0.EFFECTIVE_END_DATE ,'DD-MM-YYYY'),'31-12-4712',NULL,TO_CHAR(balancefeedE0.EFFECTIVE_END_DATE ,'DD-Mon-YYYY')) RES_EFFECTIVE_END_DATE

,balancefeedE0.LAST_UPDATED_BY RSC_LAST_UPDATED_BY
,balancefeedE0.LAST_UPDATE_DATE RSC_LAST_UPDATE_DATE
,balancefeedE0.CREATED_BY RSC_CREATED_BY
,balancefeedE0.CREATION_DATE RSC_CREATION_DATE
,NULL RSC_LEDGER_ID
...
,PAY_BALANCE_FEEDS_F balancefeedE0
,PAY_BALANCE_TYPES_VL balancetypeEO
,(SELECT NAME
...
AND elementsE0.ELEMENT_TYPE_ID = balancefeedE0.ELEMENT_TYPE_ID
AND balancefeedE0.BALANCE_TYPE_ID = balancetypeEO.BALANCE_TYPE_ID
AND elementsE0.EFFECTIVE_START_DATE = balancefeedE0.EFFECTIVE_START_DATE

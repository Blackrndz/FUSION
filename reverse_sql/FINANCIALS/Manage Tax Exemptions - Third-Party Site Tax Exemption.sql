...
SELECT
	(SELECT PartySitePEO.PARTY_SITE_NAME
	FROM HZ_PARTY_SITES PartySitePEO
	WHERE ZxPartyTaxProfile.party_id = PartySitePEO.party_site_id
	) RES_PARTY_SITE_NAME
,(SELECT PartySitePEO.PARTY_SITE_NUMBER
	FROM HZ_PARTY_SITES PartySitePEO
	WHERE ZxPartyTaxProfile.party_id = PartySitePEO.party_site_id
	AND rownum                       = 1
	) RES_PARTY_SITE_NUMBER
...
,(SELECT LocationPEO.ADDRESS1 || ',' || LocationPEO.ADDRESS2 || ',' || LocationPEO.ADDRESS3 || ',' ||LocationPEO.CITY
...
,(SELECT meaning
	...
	WHERE lookup_type = 'ZX_TP_PARTY_USAGE'
	AND lookup_code   = ZxExemptions.PARTY_TYPE_FLAG
...
,ZxExemptions.PROTOCOL_NUMBER_SEQ RES_PROTOCOL_NUMBER
,TO_CHAR(ZxExemptions.RECORDING_DATE,'DD-Mon-YYYY') RES_RECORDING_DATE
,TO_CHAR(ZxExemptions.ISSUE_DATE,'DD-Mon-YYYY')  RES_LETTER_DATE
,(SELECT meaning
	FROM fnd_lookup_values_tl
	WHERE lookup_type = 'ZX_EXEMPTION_TYPE'
...
,DECODE(ZxExemptions.APPLY_TO_LOWER_LEVELS_FLAG,'Y','Yes','No') RES_APPLY_TO_LOWER_LEVELS
,TO_CHAR(ZxExemptions.SUSPEND_FROM_DATE,'DD-Mon-YYYY')  RES_SUSPENSION_OR_REVOCATION_F
,TO_CHAR(ZxExemptions.SUSPEND_TO_DATE,'DD-Mon-YYYY') RES_SUSPENSION_TO_DATE
,ZxExemptions.LIMIT_AMOUNT RES_EXEMPTION_AMOUNT 
,(SELECT meaning
	FROM fnd_lookup_values_tl
	WHERE lookup_type = 'JEIT_CLAUSE_REF'
	AND lookup_code   = ZxExemptions.CLAUSE_REFERENCE
	AND LANGUAGE = USERENV('LANG')
	) RES_CLAUSE
,DECODE(ZxExemptions.ISSUE_FLAG,'Y','Yes','No') RES_PREPARE_LETTER_OF_INTENT
,DECODE(ZxExemptions.PRINT_FLAG,'Y','Yes','No') RES_LETTER_OF_INTENT_READY_TO_
,DECODE(ZxExemptions.CUSTOM_CHARGE_FLAG,'Y','Yes','No') RES_ADDRESS_TO_CUSTOMS_OFFICE

 ...
 * $HeadURL: https://svn03.rapid4cloud.com/svn/a/dev/rapidesuite/controldata/FUSION_11.1.13/trunk/core/reverse_sql/FINANCIALS/Configure%20Legislations%20for%20Human%20Resources%20-%20Configure%20Legislations%20for%20Human%20Resource.sql $:
 * $Id: Configure Legislations for Human Resources - Configure Legislations for Human Resource.sql 78239 2022-04-07 15:37:05Z pisan.jariyasettachok $:
 * ****************************************************************************
 * Description:
 * ************************************************************************** */


WITH TAX_YEAR AS (SELECT LEGISLATION_CODE,RULE_MODE 
	FROM PAY_LEGISLATION_RULES 
	WHERE RULE_TYPE = 'L')
,PAYROLL_RELATION AS (SELECT LEGISLATION_CODE,RULE_MODE 
	FROM PAY_LEGISLATION_RULES 
	WHERE RULE_TYPE = 'I')
...
,(CASE WHEN INSTR((SELECT RULE_MODE FROM TAX_YEAR WHERE LEGISLATION_CODE = payINSTALLLEGISEO.LEGISLATION_CODE),'/') = 0 
	THEN 
	(SELECT SUBSTR(RULE_MODE,INSTR(RULE_MODE,'-')+1,LENGTH(RULE_MODE)) 
		FROM TAX_YEAR 
		WHERE LEGISLATION_CODE = payINSTALLLEGISEO.LEGISLATION_CODE)
	ELSE
	(SELECT SUBSTR(RULE_MODE,INSTR(RULE_MODE,'/')+1,LENGTH(RULE_MODE)) 
		FROM TAX_YEAR 
		WHERE LEGISLATION_CODE = payINSTALLLEGISEO.LEGISLATION_CODE)
	END) RES_TAX_YEAR_START_DATE_MONTH 
,(CASE WHEN INSTR((SELECT RULE_MODE FROM TAX_YEAR WHERE LEGISLATION_CODE = payINSTALLLEGISEO.LEGISLATION_CODE),'/') = 0 
	THEN 
	(SELECT SUBSTR(RULE_MODE,1,INSTR(RULE_MODE,'-')-1) 
		FROM TAX_YEAR 
		WHERE LEGISLATION_CODE = payINSTALLLEGISEO.LEGISLATION_CODE)
	ELSE
	(SELECT SUBSTR(RULE_MODE,1,INSTR(RULE_MODE,'/')-1) 
		FROM TAX_YEAR 
		WHERE LEGISLATION_CODE = payINSTALLLEGISEO.LEGISLATION_CODE) 
	END) RES_TAX_YEAR_START_DATE_DATE
,(SELECT NAME 
	FROM FND_CURRENCIES_VL 
	WHERE CURRENCY_CODE = fndTERREO.CURRENCY_CODE
	) RES_DEFAULT_CURRENCY
,(SELECT MEANING 
	FROM FND_LOOKUP_VALUES 
	WHERE LOOKUP_CODE = payRELAEO.RULE_MODE
	AND LOOKUP_TYPE = 'PAY_TMPLT_LEG_RULE_REHIRE' 
	AND LANGUAGE = USERENV('LANG')
	AND SYSDATE BETWEEN START_DATE_ACTIVE AND NVL(END_DATE_ACTIVE ,TO_DATE('31-12-4712','DD-MM-YYYY'))
	) RES_PAYROLL_RELATIONSHIP_RULE
